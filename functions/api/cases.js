import { insertCase, listCompletedCaseReferences, listReferenceDocs, replaceAutoArchivedReferencesForCase, updateCaseReportById } from '../_shared/db.js';
...
export async function onRequestPost(context) {
  ...
  try {
    const [referenceDocs, previousReports] = await Promise.all([
      listReferenceDocs(context.env.DB),
      listCompletedCaseReferences(context.env.DB, 3),
    ]);
    ...
    await replaceAutoArchivedReferencesForCase(context.env.DB, qualityCase, report);
